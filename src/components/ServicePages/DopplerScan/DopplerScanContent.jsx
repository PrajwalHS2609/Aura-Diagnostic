"use client";

import React, { useState } from "react";

const DopplerScanContent = () => {
  const [expanded, setExpanded] = useState(false);

  return (
    <div className="contentHead">
      <div className="contentHead-wrapper">
        <div className="contentHead-content">
          <h1> Doppler Scan in Girinagar, Bangalore – Aura Diagnostics</h1>
          <p>
            A Doppler scan is one of the most essential diagnostic imaging
            methods used to evaluate blood flow in arteries, veins, and fetal
            vessels. In Girinagar, Bangalore, many individuals—ranging from
            expectant mothers to those needing vascular assessments—search for
            reliable facilities offering high-quality Doppler examinations. A
            Doppler study provides real-time insights into how blood circulates
            through various parts of the body, ensuring early detection of
            abnormalities and accurate clinical monitoring.
          </p>
          <p>
            Whether you are advised to undergo a Vascular Doppler scan in
            Girinagar, a Pregnancy Doppler scan in Girinagar, or a Carotid
            Doppler test in Girinagar, choosing a center equipped with advanced
            technology and trained radiologists is crucial for dependable
            results.
          </p>
        </div>
        {expanded && (
          <>
            <div className="contentHead-content">
              <h2>
                Comprehensive Doppler Scan Services Available in Girinagar
              </h2>
              <h3>1. Vascular Doppler Scan in Girinagar</h3>
              <p>
                A vascular Doppler scan helps evaluate blood flow in the major
                arteries and veins. It is commonly recommended for individuals
                experiencing leg pain, swelling, suspected varicose veins,
                peripheral artery disease, or circulation disorders. A Vascular
                Doppler scan in Girinagar checks for blockages, narrowing of
                vessels, blood clots, and venous insufficiency. This test is
                especially vital for diabetic patients and individuals with
                long-term cardiovascular conditions.
              </p>

              <h3>2. Pregnancy Doppler Scan in Girinagar</h3>
              <p>
                A Pregnancy Doppler scan in Girinagar is prescribed during the
                second or third trimester to assess the well-being of the fetus.
                It examines blood flow in the umbilical artery, uterine artery,
                fetal brain vessels, and placenta. This scan is particularly
                important for pregnancies involving high blood pressure,
                gestational diabetes, growth restrictions, or reduced fetal
                movements. It helps determine fetal oxygen supply, placental
                efficiency, and overall fetal development.
              </p>

              <h3>3. Fetal Doppler Scan in Girinagar</h3>
              <p>
                A Fetal Doppler scan in Girinagar enables detailed monitoring of
                blood flow in specific fetal organs and vessels. It is
                frequently used in high-risk pregnancies to evaluate fetal
                health, detect abnormalities, and monitor fetal responses to
                treatment. This test plays a vital role in identifying fetal
                growth issues and ensuring timely medical intervention when
                required.
              </p>
              <h3>4. Carotid Doppler Test in Girinagar</h3>
              <p>
                A Carotid Doppler test in Girinagar examines the carotid
                arteries located in the neck to assess blood flow to the brain.
                This test helps detect plaque buildup, blood clots, or narrowing
                of arteries that may increase the risk of stroke. It is commonly
                recommended for individuals with hypertension, cholesterol
                issues, diabetes, or a family history of cardiovascular
                diseases.
              </p>
              <h3>5. Arterial Doppler Scan in Girinagar</h3>
              <p>
                An Arterial Doppler scan in Girinagar evaluates circulation in
                the arms and legs. It is crucial for diagnosing peripheral
                arterial disease (PAD), arterial blockages, or reduced blood
                circulation. Early diagnosis helps in preventing complications
                such as tissue damage, ulcers, or pain caused by restricted
                blood supply.
              </p>
              <h3>6. Venous Doppler Scan in Girinagar</h3>
              <p>
                A Venous Doppler scan in Girinagar focuses on veins to detect
                deep vein thrombosis (DVT), venous reflux, and varicose veins.
                This test is essential for individuals experiencing leg
                swelling, pain, or discomfort, particularly after long periods
                of immobility or surgery.
              </p>
              <h3>7. Obstetric Doppler Scan in Girinagar</h3>
              <p>
                Pregnant women often require an Obstetric Doppler scan in
                Girinagar to monitor fetal circulation and placental health.
                This scan helps evaluate fetal growth patterns, organ
                development, and blood supply to ensure timely identification of
                any complications.
              </p>
              <h3>8. Ultrasound Doppler Imaging in Girinagar</h3>
              <p>
                Modern Ultrasound Doppler imaging in Girinagar uses
                high-frequency sound waves to provide clear, real-time images of
                blood flow. It is non-invasive and does not require any
                injections or radiation exposure, making it completely safe for
                both adults and pregnant women.
              </p>
              <h3>9. Whole Body Doppler in Girinagar</h3>
              <p>
                A Whole body Doppler in Girinagar is recommended in cases where
                systemic blood circulation issues are suspected. This scan helps
                assess multiple arteries and veins, ensuring a holistic
                understanding of a patient’s vascular health. It is useful for
                individuals with chronic medical conditions such as
                hypertension, diabetes, or heart disease.
              </p>
            </div>
            <div className="contentHead-content">
              <h2>When Should You Consider a Doppler Scan?</h2>
              <p>
                You may be advised to undergo a Doppler study if you experience:
              </p>
              <ul>
                <li> Persistent leg pain or swelling</li>
                <li> Suspected blood clots</li>
                <li> Varicose veins</li>
                <li> Reduced fetal movements</li>
                <li> High-risk pregnancy conditions</li>
                <li> Dizziness or suspected carotid artery issues</li>
                <li> Cold feet or hands indicating poor circulation</li>
                <li> Unexplained limb numbness</li>
              </ul>
              <p>
                A Doppler scan provides accurate diagnostic information,
                enabling doctors to plan effective treatments and monitor
                disease progression with precision.
              </p>
            </div>
            <div className="contentHead-content">
              <h2>How Doppler Scans Help in Early Diagnosis</h2>
              <h3>
                Doppler ultrasound technology helps detect issues early by:
              </h3>
              <ul>
                <li> Identifying blockages in arteries and veins</li>
                <li> Monitoring fetal growth and development</li>
                <li> Detecting reduced oxygen flow to the fetus</li>
                <li> Assessing blood supply to organs</li>
                <li> Evaluating risks of stroke or cardiac complications</li>
                <li> Detecting abnormalities in circulatory conditions</li>
              </ul>
              <p>
                This early detection improves treatment outcomes and helps
                prevent complications in both vascular and pregnancy-related
                cases.
              </p>
              <p>
                A Doppler scan is a vital diagnostic tool for evaluating blood
                flow in various parts of the body and ensuring fetal well-being
                in pregnancy. Whether you need a Vascular Doppler scan in
                Girinagar, a Fetal Doppler scan in Girinagar, a Carotid Doppler
                test in Girinagar, or comprehensive Ultrasound Doppler imaging
                in Girinagar, choosing an advanced and reliable diagnostic
                center ensures accurate results and better clinical outcomes.
              </p>
            </div>
          </>
        )}
        <button className="readMoreBtn" onClick={() => setExpanded(!expanded)}>
          {expanded ? "Read Less ▲" : "Read More ▼"}
        </button>
      </div>
    </div>
  );
};

export default DopplerScanContent;
